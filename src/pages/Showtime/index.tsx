...
const Showtime = () => {
  ...
  const [selectedSeat, setSeletedSeat] = useState<TSeat | null>(null);
  ...
  console.log("🚀 ~ file: index.tsx:15 ~ Showtime ~ showtime:", showtime);

  const [bookingSeat, { isLoading: isLoadingBooking }] =
    useBookingSeatMutation();
  ...
  useEffect(() => {
    const currentSeatsHeld =
      showtime?.seatsHeld.find((s) => s.user === profile._id)?.seats || [];
    setSeletedSeats(currentSeatsHeld);
  }, [showtime]);
  ...
  const handleSelectSeats = (seat: TSeat) => {
    setSeletedSeat(seat);
    bookingSeat({
              ...
              >
                {showtime?.room?.seats.map((seat: any) => (
                  <div key={seat._id} onClick={() => handleSelectSeats(seat)}>
                    ...
                    >
                      {isLoadingBooking && selectedSeat?._id === seat._id ? (
                        <div role="status">
                          <svg
                            aria-hidden="true"
